...
class _Node2VecHyperGCN(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, edge_index: torch.Tensor, num_nodes: int):
        ...
        self.hypergraph_node_conv = HypergraphConv(in_features, in_features // 2, attention_mode="edge")
        self.linear = nn.Linear(in_features // 2, out_features)
        self.aggr_2 = torch_geometric.nn.aggr.MaxAggregation()
    
    def forward(self, edge_index: torch.Tensor, sigmoid: bool = False) -> torch.Tensor:
        y = self.node2vec()
        y = self.dropout(y)
        ...
        y = self.linear(y)
        y = nn.functional.leaky_relu(y)
        y = self.aggr_2(y[edge_index[0]], edge_index[1])
        if sigmoid:
...
class Node2VecHyperGCN(CustomEstimator):
    ...
    def fit(self, X: np.ndarray, motifs: np.ndarray, X_validation, y_validation_e, motifs_validation, y_validation_m):
        ...
        for epoch in range(epochs):
            ...
            for batch in training_loader:
                ...
                optimizer.zero_grad()
                y_pred_e = self.model(nei)
                y_pred_m = y_pred_e[motifs_].prod(axis=1)
            ...
            if epoch % 2 == 0:
                with torch.no_grad():
                    ...
                    y_pred_m = y_pred_e[motifs_validation].prod(axis=1)
    ...
    def predict(self, X: np.ndarray, motifs: np.ndarray) -> np.ndarray:
        ...
        y_pred_e = self.model(nei, sigmoid=True)
